...
import sys
"""事件"""
def check_keydown_events(event,settings,screen,wbc_list,bullets,index,):
    """响应按键"""
    if event.key == pygame.K_RIGHT:
        # 向右移动
        wbc_list[0].moving_right = True
    elif event.key == pygame.K_LEFT:
        # 向左移动
        wbc_list[0].moving_left = True
    elif event.key == pygame.K_UP:
        # 向上移动
        wbc_list[0].moving_up = True
    elif event.key == pygame.K_DOWN:
        # 向下移动
        wbc_list[0].moving_down = True
    elif event.key == pygame.K_SPACE:
        # 根据index产生子弹类型
        settings.bullet_sound.play()
        fire_bullet(settings,screen,wbc_list[0],bullets,index+1,settings.bullet_life_list[index+1])
        ...
        if len(wbc_list) > 1:
            wbc_list[1].moveRight()
    elif event.key == pygame.K_c:
        if len(wbc_list) > 1:
            settings.bullet_sound.play()
            fire_bullet(settings, screen, wbc_list[1], bullets, index + 1, settings.bullet_life_list[index + 1])
    elif event.key == pygame.K_b:
        fire_bullet(settings,screen, wbc_list[0],bullets,1,settings.bullet_life_list[1])
    elif event.key == pygame.K_v:
        fire_bullet(settings,screen, wbc_list[0],bullets,2,settings.bullet_life_list[2])
    elif event.key == pygame.K_q:
        sys.exit()
    elif event.key == pygame.K_q:
        sys.exit()

def check_keyup_events(event,wbc_list):
    """响应松开"""
    if event.key == pygame.K_RIGHT:
        # 向右移动
        wbc_list[0].moving_right = False
    elif event.key == pygame.K_LEFT:
        # 向左移动
        wbc_list[0].moving_left = False
    elif event.key == pygame.K_UP:
        # 向上移动
        wbc_list[0].moving_up = False
    elif event.key == pygame.K_DOWN:
        # 向下移动
        wbc_list[0].moving_down = False

def check_events(settings,screen,wbc_list,bullets,index,):
    """响应按键事件"""
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:
            check_keydown_events(event,settings,screen,wbc_list,bullets,index)
        elif event.type == pygame.KEYUP:
            check_keyup_events(event,wbc_list)

"""绘制屏幕"""
def updata_screen(screen,settings,background,ncovs_list,wbc_list,bullets,sb,gifts,):
    """更新屏幕上的图像，并切换到新的屏幕"""
    # #每次循环时候重绘屏幕
    ...
    # 绘制白细胞
    i = settings.wbc_image_index
    for wbc in wbc_list:
        wbc.blitme_play(settings.wbc_image_path[i])
        i +=1
    # wbc.blitme()
    # 绘制子弹
...
def create_boss(settings, screen,stats):
    ...
    ncovs_list = []
    ncovs = Group()
    ncov = nCoV(settings, screen, img_path=settings.ncov_image_path[i], index=-1, stats=stats,
                    life=500*(settings.rank_in+1), speed=3)
    ncovs.add(ncov)
...
def fire_bullet(settings,screen,wbc,bullets,img,life):
    """如果还没达到限制，就发射一颗子弹"""
    if img ==1 and len(bullets)>10:
        img =0
    if img == 2 and len(bullets) > 2:
        img = 0
...
"""update 白细胞，病毒，子弹"""
def updata_wbc(wbc_list,ncovs_list,stats,screen,settings,sb):
    """白细胞移动"""
    # 白细胞移动
    for wbc in wbc_list:
        wbc.update()
...
def reset(ncovs_list,bullets,stats,sb,settings,screen,gifts):
    ...
    if len(ncovs_list) == 0:
        ...
        # 创建新的病毒
        if stats.level%5 == 0 :
            ncovs_list = create_boss(settings, screen, stats)
        else:
...
def end_time(screen):
    ...
    for i in range(5, 1, -1):
        ...
        font_rect.right = screen_rect.right -200
        font_rect.top = 350
