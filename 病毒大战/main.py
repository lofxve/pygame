...
def rungame(score,rank_in):
    ...
    while True:
        ...
        if stats.game_active:
            # 监听事件
            gf.check_events(settings,screen,wbc,bullets,index)
            # 控制游戏最大帧率为 30
            clock.tick(30)
            # 更新白细胞位置，并对其进行与礼物和病毒的碰撞检测
            gf.updata_wbc(wbc,ncovs_list,stats,screen,settings,sb)
            # 检测白细胞与礼物之间的碰撞，index礼物编号
            collide_list = pygame.sprite.spritecollide(wbc, gifts, True)
            if collide_list:
                index = collide_list[0].index
            ...
            # 更新屏幕
            gf.updata_screen(screen,settings,background,ncovs_list,wbc,bullets,sb,gifts)
        else:
            print("游戏结束")
...
if __name__ == '__main__':
    ...
    # choices = g.buttonbox(image=image,choices=("登录", "注册","游客"), title=title)
    # if choices == "登录":
    #     login(title,)
    # elif choices == "注册":
    #     register(title,)
